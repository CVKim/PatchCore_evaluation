...
class PatchCore(BaseCore):
    def __init__(
        ...
        if max_coreset_size is not None:
            max_coreset_shape = (max_coreset_size, feature_embedder.get_feature_map_shape()[-1])
        else:
    ...
    def _backup_coreset(self):
        backup_coreset_path = os.path.join(os.path.dirname(self.coreset_path), 'coreset_backup.npy')
    ...
    def _get_coreset(self, trainloader: DataLoader, base_coreset=None) -> torch.Tensor:
        ...
        coreset_features, _ = self.sampler.run(features, 
                                               base_coreset=base_coreset)
