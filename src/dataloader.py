...
def clists():
    ...
    mvtec_clist = ["bottle","cable","capsule","carpet","grid","hazelnut","leather","metal_nut",
                   "pill","screw","tile","toothbrush","transistor","wood","zipper"]

    visa_clist = ["candle","capsules","cashew","chewinggum","fryum","macaroni1",
...
def get_dataloaders(config, data_format, data_path, batch_size):
    ...
    if data_format == 'mvtec-multiclass':
        return get_mvtec_dataloaders(data_path, batch_size, config.data.imagesize, config.data.resize, multiclass=True)
    elif data_format == 'labelme':
        return get_labelme_dataloaders(data_path, batch_size, config.data.imagesize, config.data.inputsize, config.data.overlap_ratio, config.data.roi)
    else:
        raise NotImplementedError()


def get_mvtec_dataloaders(data_path, batch_size, imagesize, resize=None, multiclass=True):
    ...
    if multiclass:
        ...
        dataloaders = [{'train': train_dataloader, 'test': test_dataloader} for test_dataloader in test_dataloaders]
...
def get_labelme_dataloaders(data_path, batch_size, imagesize, inputsize, overlap_ratio, roi):
    from src.datasets.labelme import IterablePatchDataset

    train_data_path = f"{data_path}/train"
    test_data_path = f"{data_path}/val"

    train_dataset = IterablePatchDataset(train_data_path, patch_size=imagesize, input_size=inputsize, exclude_blob_area=True, overlap_ratio=overlap_ratio, roi=roi)
    test_dataset = IterablePatchDataset(test_data_path, patch_size=imagesize, input_size=inputsize, exclude_blob_area=False, overlap_ratio=overlap_ratio, roi=roi)
