...
class IterablePatchDataset(IterableDataset):
    def __init__(self, image_dir_path, patch_size, input_size, overlap_ratio=0.0, exclude_blob_area=False, roi=None):
        ...
        self.patch_dict, self.is_anomaly, self.image_sizes, self.blob_areas_dict = self._get_data_info(roi)
    ...
    def _generate_patches(self, img, roi, blob_areas):
        ...
        for x in range(roi_x0, roi_x1, stride_x):
            for y in range(roi_y0, roi_y1, stride_y):
                ...
                if self.exclude_blob_area and self.is_overlapping_blob(patch, blob_areas):
                    continue
    ...
    def is_overlapping_blob(self, patch, blob_areas):
        ...
        patch_polygon = Polygon([(patch[0], patch[1]), 
                                 (patch[2], patch[1]), 
                                 (patch[2], patch[3]), 
                                 (patch[0], patch[3])])
    ...
    def _get_blob_areas(self, image_path):
        ...
        if os.path.exists(json_path) and image_height is not None and image_width is not None:
            try:
    ...
    def get_masks(self):
        ...
        for image_path in self.image_paths:
            ...
            masks[image_path] = self._get_mask_gt(blob_areas, image_size[0], image_size[1])
        return masks
    ...
    def process_image(self, image_path, patches):
        with Image.open(image_path) as img:
            ...
            for patch in patches:
                ...
                yield {'image': self.transform(cropped_img), 
