...
class Engine:
    ...
    def _image_infer(self) -> np.ndarray:
        ...
        with tqdm(
            self.test_dataloader, desc="Inferring...",
        ) as data_iterator:
...
def _fit_coreset_by_partition_train(
    ...
    for i in tqdm(
        ...
        if i == 0:
            ...
            coreset_model.incremental_fit(_tmp_dataloader, set_predictor=False, save_coreset=True)
            _save_integer(train_status_file_path, i + 1)
        else:
            coreset_model.incremental_fit(_tmp_dataloader, set_predictor=False, save_coreset=False)
