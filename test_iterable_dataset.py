...
def worker_init_fn(_):
    ...
    dataset.image_paths = dataset.image_paths[worker_id * split_size: (worker_id + 1) * split_size]

def test_dataloader(prefetch_factor, num_workers=4, batch_size=4, iterable=True, pin_memory=False, use_worker_init_fn=False, shuffle=False):
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers, prefetch_factor=prefetch_factor, worker_init_fn=_worker_init_fn, pin_memory=pin_memory, shuffle=shuffle)
    ...
    print(f"prefetch_factor={prefetch_factor}, num_workers={num_workers}, iterable={iterable}, pin_memory={pin_memory}, use_worker_init_fn={use_worker_init_fn}: Time taken = {end_time - start_time:.2f} seconds")
